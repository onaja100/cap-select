...
const ResultItem: React.FC<ResultItemProps> = ({ label, value, unit, highlight = false }) => {
  return (
    <div className={`
      p-4 sm:p-5 rounded-xl transition-all duration-300 hover:shadow-lg relative overflow-hidden
      ${highlight 
        ? 'bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-300 shadow-lg' 
        : 'bg-white/70 backdrop-blur-sm border border-gray-200 hover:border-gray-300'
      }
    `}>
      {highlight && (
      ...
      <div className="text-sm font-medium text-gray-600 mb-1.5">{label}</div>
      <div className={`text-xl sm:text-2xl font-bold flex items-baseline ${highlight ? 'text-green-700' : 'text-gray-800'}`}>
        <span>{value.toLocaleString()}</span>
        <span className={`text-sm font-medium ml-2 ${highlight ? 'text-green-600' : 'text-gray-500'}`}>
          {unit}
        </span>
      </div>
    </div>
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-4 sm:p-6 mb-6 sm:mb-8 hover:shadow-2xl transition-all duration-300">
      <div className="flex items-center mb-4 sm:mb-6">
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-lg p-2 sm:p-2.5 mr-3 shadow-lg">
      ...
      <div className="mt-4 sm:mt-6 p-4 sm:p-5 bg-gradient-to-r from-amber-50 to-yellow-50 rounded-xl border-l-4 border-amber-400 shadow-sm">
        <div className="flex items-start">
          ...
          <div className="ml-4">
            <h4 className="text-amber-800 font-semibold mb-2">คำแนะนำสำคัญ</h4>
